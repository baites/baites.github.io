...
Given an sorted array $A[0..S-1]$ where $S$ is the array size, its median is defined as:
...
There are two sorted arrays $A[0..S_A-1]$ and $B[0..S_B-1]$ of size $S_A$ and $S_B$ respectively. Find the median of the two sorted arrays with a time complexity $O(\log(S_A+S_B))$ and in constant space complexity. You may assume $A$ and $B$ cannot be both empty.[^3]
{% include statement/end %}
...
def MergeArrays(A, B):
    ...
    while iA < SA or iB < SB:
        AIsSmaller = (iA != SA) && (iB == SB || A[iA] < B[iB])
        if AIsSmaller:
            F[iA+iB] = A[iA]
            iA += 1
        else:
...
We define the $\text{Prefix}(A,n)$ where $A[0..S_A-1]$ is an array and $n$ an integer such as $n \in [0, S_A]$, as the array form by the first $n$ elements of $A$. In particular, we say that $\text{Prefix}(A,0)$ is an empty array, and also that $A =  \text{Prefix}(A,S_A)$.

In a complementary way, we write $\text{Suffix}(A,n)$ as the resulting array after removing the first $n$ elements of $A$ such as $A = \text{Prefix}(A,n) + \text{Suffix}(A,n)$ for any value of $n \in [0, S_A]$. From this definition it should be clear that $A = \text{Suffix}(A,0)$, and that $\text{Suffix}(A,S_A)$ is an empty array.

We would like to remark that if an array $A$ is already sorted, then for any the value of $n \in [0, S_A]$

$$
A = \text{Prefix}(A,n) + \text{Suffix}(A,n) = \text{Prefix}(A,n) \oplus \text{Suffix}(A,n) \text{ (p.6)}
$$

and therefore by definition (d.4), its prefix is always less or equal than its suffix or:

$$
\text{Prefix}(A,n) \leq \text{Suffix}(A,n) \text{ for all } n \in [0, S_A] \text{ (p.7).}
$$
...
$$
\text{Prefix}(A, n_A) \oplus \text{Prefix}(B, n_B) = \text{Prefix}(A \oplus B, n_A + n_B) \text{ (c.10)}
$$
...
<center><p>Prefix (head) and suffix (tail) relationships for a sort-merge array.</p></center>

The merge-sort array also has a complementary suffix relationship in where for those $n_A$ and $n_B$ complying with the previous condition (c.10) we can write

$$
\begin{aligned}
A \oplus B &= \left[\text{Prefix}(A, n_A) \oplus \text{Suffix}(A, n_A)\right] \oplus \left[\text{Prefix}(B, n_B) \oplus \text{Suffix}(B, n_B)\right] \text{ (using p.6 on A and B)} \newline
           &= \left[\text{Prefix}(A, n_A) \oplus \text{Prefix}(B, n_B)\right] \oplus \left[\text{Suffix}(A, n_A) \oplus \text{Suffix}(B, n_B)\right] \text{ (because of p.2 and p.3)} \newline
           &= \text{Prefix}(A \oplus B, n_A + n_B) \oplus \left[\text{Suffix}(A, n_A) \oplus \text{Suffix}(B, n_B)\right] \text{ (using c.10 assumed to be true)}
\end{aligned}
$$

Due to (p.6) we know also that $A \oplus B =  \text{Prefix}(A \oplus B, n) \oplus \text{Suffix}(A \oplus B, n)$ for any $n \in [0, S_A + S_B]$. But then because of (p.9) then it has to true that

$$
\text{Suffix}(A, n_A) \oplus \text{Suffix}(B, n_B) = \text{Suffix}(A \oplus B, n_A + n_B) \text{ (c.12)}
$$

# The merge-sort map lemma

As formulated, the prefix-suffix relationships (c.10) and (c.12) presented in the previous section is not enough to help us building a more efficient way of solving the problem. However, the following lemma provides us with a new set of prefix-suffix relationships that can be verified efficiently.

{% include statement/lemma name="merge-sort map" %}
Given two sorted arrays $A[0..S_A-1]$ and $B[0..S_B-1]$ and two positive integer $n_A$ and $n_B$, then they comply with the prefix-suffix conditions (c.10) (and therefore also (c.12)) if and only if it is true that

$$
\begin{aligned}
\text{Prefix}(A, n_A) &\leq \text{Suffix}(B, n_B) \text{ (c.13) and} \newline
\text{Prefix}(B, n_B) &\leq \text{Suffix}(A, n_A) \text{ (c.14)}
\end{aligned}
$$
{% include statement/end %}
...
$$
\begin{aligned}
\text{Prefix}(A, n_A) \oplus \text{Prefix}(B, n_B) &=    \text{Prefix}(A \oplus B, n_A + n_B) \text{ (using c.10 assumed to be true)} \newline
                                                   &\leq \text{Suffix}(A \oplus B, n_A + n_B) \text{ (using p.7)} \newline
                                                   &\leq \text{Suffix}(A, n_A) \oplus \text{Suffix}(B, n_B) \text{ (using c.12 implied by c.10)}.
\end{aligned}
$$

where the first inequality is because for sorted array its prefix is always less than or equal to its suffix. Now property (p.7) is also true for sorted arrays $X = A \text{ or } B$ and therefore $\text{Prefix}(X, n_X) \leq \text{Suffix}(X, n_X)$ for $n_X \in [0, S_X]$. As result, the above expression can only be true if only if the conditions (c.13) and (c.14) of the lemma are also true.

Now let's show the other way around and assume that two integers $n_A$ and $n_B$ comply with the conditions (c.13) and (c.14). In the case that $n_A + n_B = 0$ then by definition $\text{Prefix}(A \oplus B, n_A + n_B)$, $\text{Prefix}(A, n_A)$, and $\text{Prefix}(B, n_B)$ are all empty arrays and therefore condition (c.10) is trivially true.

Alternative, if $n_A + n_B = n$ where $n > 0$ we can select the first element of the $\text{Prefix}(A \oplus B, n_A + n_B)$ that is the smallest element of $A \oplus B$. This element must be present in some of the following four arrays $\text{Prefix}(A, n_A)$, $\text{Prefix}(B, n_B)$, $\text{Suffix}(A, n_A)$, or $\text{Suffix}(B, n_B)$. However, it cannot be present in the suffix of $A$ or $B$ because this would contradict the condition (c.13) or (c.14) that are assumed to be true. Therefore, the first element of $\text{Prefix}(A \oplus B, n_A + n_B)$ has to be in either $\text{Prefix}(A, n_A)$ or $\text{Prefix}(B, n_B)$. We can then remove the first element from $\text{Prefix}(A \oplus B, n_A + n_B)$, and from $\text{Prefix}(A, n_A)$ or $\text{Prefix}(B, n_B)$, allowing us to do the same argument in the case $n_A + n_B = n - 1$. It is possible to repeat this argument until $\text{Prefix}(A \oplus B, n_A + n_B)$ is an empty array. But then, all the elements of $\text{Prefix}(A \oplus B, n_A + n_B)$ come from either $\text{Prefix}(A, n_A)$ or $\text{Prefix}(B, n_B)$ implying condition (c.10) is true.
...
{% highlight python %}
def IsNotPrefixSuffixCondition(self, X, Y, nX, nY):
    """Check for the negative of the prefix-suffix condition."""
...
def MergeSortMap(self, A, B, n):
    ...
    while 1:
        ...
        if nB > SB:
            left = nA + 1
        # Check if first prefix/suffix condition fails
        # meaning that nA is too large
        elif self.IsNotPrefixSuffixCondition(A, B, nA, nB):
            right = nA - 1
        # Check if second prefix/suffix condition fails
        # meaning that nA is too small
        elif self.IsNotPrefixSuffixCondition(B, A, nB, nA):
            left = nA + 1
...
$$
\text{Prefix}(A, n_A) \oplus \text{Prefix}(B, n_B) \leq \text{Suffix}(A, n_A) \oplus \text{Suffix}(B, n_B) \text{ (c.15)}
$$
...
$$
\begin{aligned}
A \oplus B &= \text{Prefix}(A \oplus B, n_A+n_B) \oplus \text{Suffix}(A \oplus B, n_A+n_B) \left(\text{using p.6 on } A \oplus B\right) \newline
           &= \left[\text{Prefix}(A, n_A) \oplus \text{Suffix}(A, n_A)\right] \oplus \left[\text{Prefix}(B, n_B) \oplus \text{Suffix}(B, n_B)\right] \text{ (using p.6 on A and B)} \newline
           &= \left[\text{Prefix}(A, n_A) \oplus \text{Prefix}(B, n_B)\right] \oplus \left[\text{Suffix}(A, n_A) \oplus \text{Suffix}(B, n_B)\right] \text{ (because of p.2 and p.3)} \newline
           &= \left[\text{Prefix}(A, n_A) \oplus \text{Prefix}(B, n_B)\right] \textcolor{red}{+} \left[\text{Suffix}(A, n_A) \oplus \text{Suffix}(B, n_B)\right] \text{ (using c.15 and d.4).}
\end{aligned}
$$

But then this implies by definition that the first(last) $n_A+n_B$ elements of $A \oplus B$ (its prefix and suffix) are sorted concatenated the prefixes(suffixes) of $A$ and $B$, meaning therefore that (c.10) and (c.12) are true.
