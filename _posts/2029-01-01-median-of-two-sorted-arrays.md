...
At this point I asked myself, are there any conditions for a pair of \\|\lbrace(i_1, i_2)\rbrace\\| such as they are part of the merge map ? To answer that question I came up with the following lemma.

> **Lemma (merge map)**: Given two sorted arrays \\|A_1[0..S_1-1]\\| and \\|A_2[0..S_2-1]\\| and the resulting sorted array after merging them \\|A(0..S-1)\\| so that \\|S = S_1 + S_2\\|, and the indexes \\|i_1, i_2\\| complying with the following conditions
...
First lets show this condition is an invariant of the merge step **MergeMedianTwoSortedArrays** algorithm. This can be done by induction:

1. By definition the condition is true for \\|i_1 = i_2 = 0\\|.
2. Assuming \\|i_1 = S_1\\| and \\|i_2 < S_2\\| the algorithm will update \\|i_2 \rightarrow i_2 + 1\\|. As result we have two possible cases that keep invariant the lemma conditions for \\|i_2 + 1\\|:
    * either \\|(i_2 + 1) = S2\\|
...
4. Now if \\|i_1 < S_1\\| and \\|i_2 < S_2\\| then the algorithm can make two possible updates:
    * if \\|A_1[i_1] < A_2[i_2]\\| resulting in the following update \\|i_1 \rightarrow i_1 + 1\\| keeping the lemma conditions invariant:
        * \\|A_1[(i_1+1)-1] < A_2[i_2]\\| and
        * \\|A_2[i_2-1] \leq A_1[i_1] \leq A_1[(i_1+1)]\\| because \\|A_2\\| is sorted.
    * the similar reasoning applies to the case of having \\|A_1[i_1] \geq A_2[i_2]\\| and resulting in the following update \\|i_2 \rightarrow i_2 + 1\\|.

### Condition to find the median in the merge map
