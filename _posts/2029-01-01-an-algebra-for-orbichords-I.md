...
$$ E_{\lambda}[h,a] = \left\{g \in G | \lambda_g(a) = \lambda_h(a)\right\}$$

is the set of all equivalent actions tp $\lambda_h(a)$ from $h \in G$ and $a \in X$.
{% include statement/end %}

**Note:** For most part I annotate this set as $E[h,a]$ by dropping the dependency with action function $\lambda$ when it is clear by context.

By definition, this set have the following properties

* $h \in E[h, a]$
* $E[G, a] = G$
* $E_{\lambda}[e, a] = \text{Stab}_{\lambda}(a) = G_a$.
...
The set $D_a \subset G$ is a fundamental domain generated by $a \in G$ if
...
The proposal can also be interpreated as elements from foundamental domain are not images of each other.

The set of all points in $X$ reacheable from $a \in X$ plays special role resulting in the following definition.

{% include statement/definition name="fundamental domain orbits" %}
I say that any point $x \in X$ is reacheable by action $(X, G, \lambda)$ if there is a $g \in D_a$ such $x = \lambda_g(a)$, or in words, there is an action that transform the domai1n generator $a$ into $x$, meaning $x$ is part of orbit $\lambda_{D_a}(a)$.
{% include statement/end %}
...
A map that takes any element $g \in G$ into a element of $D_a$ that has equivalent action.
...
* $\lambda_{\pi_a(g)}(a) = \lambda_{g}(a)$ for any $g \in G$.

{% include statement/lemma name="fundamental orbits are just orbits" markdown="block" %}
Due to fact that for any $g \in G$ we have $\lambda_{\pi_a(g)}(a) = \lambda_{g}(a)$, it follows that the fundamental orbit $\lambda_{D_a}(a) = \lambda_G(a)$!
{% include statement/end %}

## Fundamental actions

The next logical step is to add more structure to the monion action to recover some of the benefits of groups action composition. Using fundamental map we can create a new composition rule and thereofore a new type of action.
...
It is important to understand it is sufficient for composition rule to be defined only for actions over the generator of the fundamental domain $a \in X$. This is because it easy to extend this rule to any point in the orbit $\lambda_G(a)$

{% include statement/lemma name="composition of action in orbit" markdown="block" %}
For any $x \in \lambda_G(a)$ the composition between two actions is

$$ \lambda_g \circ \lambda_h(x) = \lambda_{g\pi_a(hk)}(a) $$

for any $g,h \in G$ and some $k \in D_a$.
{% include statement/end %}

To prove this lemma, I replace the value of $x$ by $\lambda_k(a)$ for some k in $D_a$ so $\lambda_h(x)$ is

$$ \lambda_h \circ \lambda_k(a) = \lambda_{hk}(a)$$

where I use the fact fundamental map $\pi_a(k) = k$ as $k$ is already a point in the funamental domain. Then it follows applying the composition rule again

$$
\begin{aligned}
\lambda_g \circ \lambda_h(x) &= \lambda_g \circ \lambda_{hk}(a) \\
                             &= \lambda_{g\pi_a(hk)}(a)
\end{aligned}
$$

that is the lemma result.

Now, let see how fundamental action include group actions.
...
I developed the necessesary tools to define a new class of actions based on groups and the identification of some member of the group as equivalent. I call this action fundamental as their definition required the notion of fundamental domains. Those domains contain all the member of a group that result in distict actions over a given element on set where actions are applied. I illustrated how all these definitions can be apply to *real pitch dyad*. I also showed group actions are particular case of fundamental actions.

In the next post, I will show the equivalent to *Transitive actions are homogeneous spaces* theorem[^4] for fundamental action. We this we generalize the most important concept of group action to fundamental ones that allow for arbitrary identification of group members as equivalent!
